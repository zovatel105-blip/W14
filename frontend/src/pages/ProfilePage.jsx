  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6">
        ...
        {/* AVATAR + MÉTRICAS (EN EL CENTRO) - DISEÑO MEDIANO HORIZONTAL */}
        <div className="bg-white rounded-xl p-4 sm:p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3 sm:gap-5">
            
            {/* LADO IZQUIERDO: Votos y Seguidores */}
            <div className="flex flex-col gap-3.5 sm:gap-4 flex-1 min-w-0">
              {/* Votos con ícono */}
              <div className="flex items-center gap-2.5">
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-blue-100 rounded-full flex items-center justify-center flex-shrink-0">
                  <Vote className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-blue-600" />
                </div>
                <div className="flex items-center gap-2 min-w-0">
                  <p className="text-base sm:text-lg font-bold text-gray-900">{displayUser.totalVotes || 0}</p>
                  <p className="text-sm sm:text-sm text-gray-600 font-medium">Votos</p>
                </div>
              </div>
              
              {/* Seguidores con ícono */}
              <div 
                className="flex items-center gap-2.5 cursor-pointer hover:bg-gray-50 rounded-lg p-2 transition-colors active:scale-95 -m-2"
                onClick={() => setShowFollowersModal(true)}
              >
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-green-100 rounded-full flex items-center justify-center flex-shrink-0">
                  <Users className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-green-600" />
                </div>
                <div className="flex items-center gap-2 min-w-0">
                  <p className="text-base sm:text-lg font-bold text-gray-900">{followersCount}</p>
                  <p className="text-sm sm:text-sm text-gray-600 font-medium">Seguidores</p>
                </div>
              </div>
            </div>

            {/* AVATAR EN EL CENTRO - IMAGEN DE PERFIL CON BORDE DEGRADADO */}
            <div className="relative w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 flex-shrink-0 mx-2 sm:mx-3">
              ...
              <button className="absolute -bottom-1 -right-1 w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 bg-blue-500 hover:bg-blue-600 rounded-full flex items-center justify-center shadow-lg transition-all duration-200 active:scale-90 z-10">
            ...
            <div className="flex flex-col gap-3.5 sm:gap-4 flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2.5 justify-end">
                <div className="flex items-center gap-2 min-w-0 order-2">
                  <p className="text-sm sm:text-sm text-gray-600 font-medium">Me gusta</p>
                  <p className="text-base sm:text-lg font-bold text-gray-900">{displayUser.totalLikes || 0}</p>
                </div>
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-red-100 rounded-full flex items-center justify-center order-1 flex-shrink-0">
                  <Heart className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-red-600" />
                </div>
              </div>
              
              {/* Seguidos con ícono */}
              <div 
                className="flex items-center gap-2.5 justify-end cursor-pointer hover:bg-gray-50 rounded-lg p-2 transition-colors active:scale-95 -m-2"
                onClick={() => setShowFollowingModal(true)}
              >
                <div className="flex items-center gap-2 min-w-0 order-2">
                  <p className="text-sm sm:text-sm text-gray-600 font-medium">Seguidos</p>
                  <p className="text-base sm:text-lg font-bold text-gray-900">{followingCount}</p>
                </div>
                <div className="w-9 h-9 sm:w-10 sm:h-10 bg-purple-100 rounded-full flex items-center justify-center order-1 flex-shrink-0">
                  <UserPlus className="w-4.5 h-4.5 sm:w-5 sm:h-5 text-purple-600" />
                </div>
              </div>
            </div>
